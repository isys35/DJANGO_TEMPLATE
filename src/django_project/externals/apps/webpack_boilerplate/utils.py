...
from .config import get_setting_value, load_config
...
def _get_bundle(bundle_name, extension, config):
    bundle = list(get_loader(config).get_bundle(bundle_name))
    if extension:
...
def get_static(asset_name, config="DEFAULT"):
    ...
    """
    return "{}{}".format(get_setting_value("STATIC_URL"), asset_name)
